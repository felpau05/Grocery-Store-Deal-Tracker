...
from models import Item, LanguageResult, Merchant, ParsedItem
...
#
def run_pipeline(raw_item: dict, merchant: Merchant) -> Item | None:
    """Run one raw item through the pipeline (or a custom subset of steps)."""
    item = ParsedItem(raw=raw_item, merchant=merchant)
...
def run_pipeline_batch(
    raw_items: list[dict], merchant_dict: dict[int, Merchant]
) -> list[Item]:
    """Run many items through the pipeline."""

    all_items = [
        run_pipeline(raw, merchant_dict[raw["merchant_id"]])
        for raw in raw_items
        if raw.get("merchant_id") in merchant_dict
    ]

    # Items are lost here (since parsing causes missing fields) should log better
